...
$Script:WorkDir = Join-Path $env:TEMP "PrinterTool_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
...
function Get-FileFromUrl {
    ...
    $destination = Join-Path $DestinationFolder $OutputFileName
    try {
        if (-not (Test-Path $DestinationFolder)) {
            New-Item -ItemType Directory -Path $DestinationFolder -Force | Out-Null
        }
        
        Write-Log "Baixando: $OutputFileName" -Type Progress
        ...
        Write-Log "Download concluído: $OutputFileName ($sizeInMB MB)" -Type Success
        return $true
    }
    catch {
...
function Invoke-FileExecution {
    ...
    try {
        switch ($extension) {
            ".exe" {
                Write-Log "Executando: $FilePath" -Type Progress
                $params = @{
                    FilePath = $FilePath
                    NoNewWindow = $false
                }
                if ($Wait) { $params.Wait = $true }
                # CORREÇÃO: só passa ArgumentList se tiver argumentos
                if ($Arguments.Count -gt 0) { $params.ArgumentList = $Arguments }
                Start-Process @params
                return $true
            }
            ".msi" {
                Write-Log "Instalando MSI: $FilePath" -Type Progress
            ...
            ".zip" {
                ...
                Write-Log "Extraindo ZIP: $FilePath" -Type Progress
            ...
            default {
                Write-Log "Abrindo pasta: $(Split-Path $FilePath)" -Type Info
...
# ==================== DATABASE DE IMPRESSORAS (COMPLETO) ====================
        ...
        'PrintWay' = @{
            ...
            PostInstall = {
                ...
                if (Test-Path $chrome) {
                    ...
                    Write-Log "Chrome não encontrado. Abra manualmente: https://app.printwayy.com" -Type Warning
                }
            }
        }
        'NAPS2' = @{
...
function Install-Printer {
    ...
    $targetFolder = if ($printer.DownloadToFolder) {
        $folder = Join-Path $env:USERPROFILE "Downloads\$PrinterKey"
    ...
    foreach ($file in $printer.Files) {
        Write-Log "[$currentFile/$fileCount] Processando: $($file.FileName)" -Type Progress
        
        $outputPath = Join-Path $targetFolder $file.FileName
        
        if (Get-FileFromUrl -Url $file.Url -OutputFileName $file.FileName -DestinationFolder $targetFolder) {
            # Se NÃO for DownloadToFolder, executa o arquivo
            if (-not $printer.DownloadToFolder) {
                if (-not (Invoke-FileExecution -FilePath $outputPath -Wait)) {
                    $allSuccess = $false
                }
            }
        } else {
    ...
    if ($printer.DownloadToFolder) {
        Write-Log "Arquivos salvos em: $targetFolder" -Type Success
    ...
    Write-Host ""
    if ($allSuccess) {
        Write-Log "Processo concluído com sucesso!" -Type Success
    } else {
        Write-Log "Processo concluído com alguns erros." -Type Warning
    }
    return $allSuccess
}

# ==================== MENUS INTERATIVOS (SEUS MENUS ORIGINAIS) ====================
...
function Show-PrinterList {
    ...
    $categories = $database.Values | Group-Object -Property Category | Sort-Object Name
    foreach ($category in $categories) {
        Write-Host "  $($category.Name):" -ForegroundColor Cyan
        $printersInCat = $database.GetEnumerator() | Where-Object { $_.Value.Category -eq $category.Name } | Sort-Object Name
        foreach ($pr in $printersInCat) {
            Write-Host "    - $($pr.Key) : $($pr.Value.Name)" -ForegroundColor White
        }
        Write-Host ""
    }
...
function Main {
    ...
    if (-not $NoAdmin -and -not (Test-Administrator)) {
        ...
        Write-Log "Algumas instalações podem falhar." -Type Warning
        Write-Host ""
    }
    
    # Conexão com internet
    if (-not (Test-InternetConnection)) {
        Write-Log "Sem conexão com internet!" -Type Error
        exit 1
    }
